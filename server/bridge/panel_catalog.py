...
CARROT_TUNING_UNAVAILABLE: frozenset[str] = frozenset({  # Dead switches: registered and (for some) still drawn in a UI, but read by
  # nothing. Hidden here, and removed from the native/sunnylink lists too, so a
  # user cannot turn a knob that does nothing.
...
PANELS: list[dict[str, Any]] = [
  ...
  },
  {
    ...
    "widgets": [{"type": "bool", "param": "AmapMapDataEnabled", "label": "Enable Amap Map Data", "offroad_only": True,
       ...
       "visible_if": {"param": "AmapMapDataEnabled", "eq": "1"},
       "desc": "Show how many traffic lights are on the route ahead. Display only; it never controls the car."},{"type": "separator"},{"type": "separator"},
